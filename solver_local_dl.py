...
from funcs import transString, calc_val
...
def word_to_letter_df(word, sura, verse, seq_index, chron_index):
    d = {'sura': sura,
         'verse': verse,
         'letters': list(word),
         'word': word,
...
def verse_to_letter_df(verse_text, sura, verse, seq_index, chron_index):
    df_list = []
    # i = 1
    for word in verse_text.split():
        df = word_to_letter_df(word, sura, verse, seq_index, chron_index)
        df_list.append(df)
...
def sura_to_letter_df(qdf, sura):
    ...
    for df_row in sura_df.itertuples():
        df_list.append(verse_to_letter_df(df_row.arabic, df_row.sura, df_row.verse,
                                          df_row.seq_index, df_row.chron_index))
    return pd.concat(df_list, ignore_index=True)
...
def verse_to_word_df(verse_text, sura, verse):
    d = {'sura': sura,
         'verse': verse,
         'word': verse_text.strip().split(),
        }
    df = pd.DataFrame(d)
    return df


def sura_to_word_df(qdf, sura):
    sura_df = qdf.query('sura == {}'.format(sura))
    df_list = []
    for df_row in sura_df.itertuples():
        df_list.append(verse_to_word_df(df_row.arabic, df_row.sura, df_row.verse))
    return pd.concat(df_list, ignore_index=True)
...
if __name__=='__main__':
    ...
    # ldf['tgv'] = ldf['letters'].apply(lambda l: calc_val(transString(l), 'tgv'))
    # ldf.to_csv('letterdf.csv')
